...
class MsspiderPipeline(object):
    ...
    def _add_md_data(self, tnx, item_list):
        insert_sql = '''
                        INSERT INTO matchdata(`mid`, `lid`, `mtid`, `jq`, `dtid`, `sq`, `mdate`)
                        VALUES(%(mid)s, %(lid)s, %(mtid)s, %(jq)s, %(dtid)s, %(sq)s, %(mdate)s)
                        ON DUPLICATE KEY UPDATE 
                        lid = values(lid), mtid=values(mtid), jq=values(jq), dtid=values(dtid),
                        sq=values(sq), mdate=values(mdate)             
                     '''
        return tnx.executemany(insert_sql, item_list)

    def add_md_data(self, item_list):
        d = self.db_pool.runInteraction(self._add_md_data, item_list)
    ...
    def process_md_item(self, item):
        try:
            ...
            if isinstance(item, MatchItem):
                ...
                self.md_item_list.append(value)
                if len(self.md_item_list) >= 100:  # self.commit_limit:
                    item_list = copy.deepcopy(self.md_item_list)
                    self.add_md_data(list(item_list))
                    self.md_item_list.clear()
    ...
    def close_spider(self, spider):
        self.add_md_data(self.md_item_list)
